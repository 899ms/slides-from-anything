...
const originalEnablePlugins = process.env.OD_CODEX_ENABLE_PLUGINS;
...
afterEach(() => {
  ...
  if (originalEnablePlugins == null) {
    delete process.env.OD_CODEX_ENABLE_PLUGINS;
  } else {
    process.env.OD_CODEX_ENABLE_PLUGINS = originalEnablePlugins;
  }
...
test('codex args disable plugins by default for OpenPPT daemon runs', () => {
  delete process.env.OD_CODEX_DISABLE_PLUGINS;
  delete process.env.OD_CODEX_ENABLE_PLUGINS;
...
test('codex args keep plugins enabled when OD_CODEX_ENABLE_PLUGINS is 1', () => {
  delete process.env.OD_CODEX_DISABLE_PLUGINS;
  process.env.OD_CODEX_ENABLE_PLUGINS = '1';
...
test('codex args still disable plugins when both enable and disable flags are set', () => {
  process.env.OD_CODEX_DISABLE_PLUGINS = '1';
  process.env.OD_CODEX_ENABLE_PLUGINS = '1';
...
test('codex legacy unsupported model choice falls back to CLI default', () => {
  assert.equal(normalizeAgentModelChoice('codex', 'gpt-5-codex'), 'default');
  assert.equal(normalizeAgentModelChoice('codex', 'gpt-5.5'), 'gpt-5.5');
  assert.equal(normalizeAgentModelChoice('claude', 'gpt-5-codex'), 'gpt-5-codex');
});
